...
tf.app.flags.DEFINE_integer('protein_no', 1,
...
def _parse_and_preprocess(x,y):
    ...
    return (x,y) #return tuple of dict of feature # name with key as provided in the feature column and label.
...
def _generate_image_and_label_batch2(image, label, batch_size=FLAGS.batch_size,shuffle=True):
    ...
    dataset= dataset.shuffle(min_after_dequeue).repeat().batch(batch_size-FLAGS.must_contain)
...
def inputs(eval_data):
    ...
    images, labels = CNNinput.inputs(eval_data=eval_data,
                                     data_dir=data_dir,
...
def train(total_loss, global_step, num_classes):
